<div class="row">
	...
	{!! Form::open(['url' => url('/abort'), 'enctype' => 'multipart/form-data', 'id' => 'submit_img', 'class' => '']) !!}
			...
			<div class="form-group row">
				<label for="image_title" class="col-12 col-form-label">{{ __('site.title') }}</label>
			...
			<div class="form-group row">
				<label for="gallery" class="col-12">{{ __('site.gallery') }}</label>
				<div class="col-12 col-md-6 col-lg-5">
...
<div class="row">
	...
	@if ($isMobile)
		<div id="files_list" class="col-12 d-flex flex-wrap justify-content-center">
...
<div class="row">
	<div class="col-12">
		...
		{!! Form::open(['url' => url('/abort'), 'id' => 'create_gal', 'class' => 'form-rosw']) !!}
                <input type="hidden" name="create_gallery" value="1"/>
                <div class="form-group row">
                    <label for="gal_title" class="col-12 gallery_title">{{ __('site.title') }}</label>
                    <div class="col-12 col-md-6 col-lg-5" style="{{-- {{ ($isMobile ? 'margin-left:-5px;' : 'margin-left:-35px;') }} --}}">
                        <input type="text" id="gal_title" class="form-control form-control-sm" name="gallery_title"/>
                    </div>
                </div>
                <div class="form-group row mt-1">
                    <div style="margin-top: -5px;" class="col-12 col-md-6 col-lg-5 d-flex justify-content-end">
		...
		<h4 class="mb-3" style="color: red;">{{ __('site.manage_galleries') }}</h4>
			@if (!empty($galleries_names))
				{!! Form::open(['url' => url('/abort'), 'id' => 'modify_gal', 'class' => '']) !!}
						...
						<button class="mt-1 btn btn-primary btn-sm" style="{{ ($isMobile ? 'margin-left: 2rem;' : 'margin-left: 12rem;') }}" type="submit">{{ __('site.apply_changes') }}</button>
						<button style="display:none; {{ ($isMobile ? 'margin-left: 2rem;' : 'margin-left: 12rem;') }}" class="btn btn-primary btn-sm" type="button" disabled>
							<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
							{{ __('site.changes_in_progress') }}
						</button>
						<button style="display:none; {{ ($isMobile ? 'margin-left: 2rem;' : 'margin-left: 12rem;') }}" type="button" class="btn btn-sm btn-success" ><i class="far fa-check-circle"></i> <b>{{ __('site.changes_done') }}</b></button>
				{!! Form::close() !!}
			@else
